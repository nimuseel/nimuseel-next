...
export const Career = () => {
  return (
    <section style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      {careerList.map((item) => (
        <div key={item.companyName} style={{ lineHeight: 1.2 }}>
          <h2 style={{ margin: 0 }}>
            {item.companyName}
            <span style={{ fontSize: 14, marginLeft: 8, color: 'gray' }}>
              {item.period}
            </span>
          </h2>
          <p>{item.position}</p>
          ...
          {item.projects.map((project) => (
            <div key={project.name}>
              ...
              <p>소개: {project.description}</p>

              <div>
                <p style={{ margin: 0 }}>작업 요약</p>
